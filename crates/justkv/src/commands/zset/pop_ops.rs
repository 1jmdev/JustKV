use crate::commands::util::{Args, eq_ascii, f64_to_bytes, int_error, wrong_args, wrong_type};
...
fn flatten_member_scores(items: Vec<(crate::engine::value::CompactKey, f64)>) -> Vec<RespFrame> {
...
fn items_to_pairs(items: Vec<(crate::engine::value::CompactKey, f64)>) -> Vec<RespFrame> {
    items
        .into_iter()
        .map(|(member, score)| {
