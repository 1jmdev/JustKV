...
pub(crate) fn xreadgroup(store: &Store, args: &Args) -> RespFrame {
    let _trace = profiler::scope("crates::commands::src::stream::claim::xreadgroup");
    if args.len() < 7 {
                    ...
                    items
                        ...
                        .map(|(key, values)| {
                            RespFrame::Array(Some(vec![
                                ...
                                RespFrame::Array(Some(super::range::format_items(values))),
                            ]))
                        })
                        .collect(),
                ))
...
pub(crate) fn xclaim(store: &Store, args: &Args) -> RespFrame {
    let _trace = profiler::scope("crates::commands::src::stream::claim::xclaim");
    if args.len() < 6 {
    ...
    match store.xclaim(&args[1], &args[2], &args[3], &ids) {
        Ok(items) => RespFrame::Array(Some(super::range::format_items(items))),
        Err(_) => wrong_type(),
    }
}

pub(crate) fn xautoclaim(store: &Store, args: &Args) -> RespFrame {
    let _trace = profiler::scope("crates::commands::src::stream::claim::xautoclaim");
    if args.len() < 6 {
    ...
    match store.xautoclaim(&args[1], &args[2], &args[3], start, count) {
        Ok((next, items)) => RespFrame::Array(Some(vec![
            ...
            RespFrame::Array(Some(super::range::format_items(items))),
            RespFrame::Array(Some(vec![])),
        ])),
        Err(_) => wrong_type(),
    }
