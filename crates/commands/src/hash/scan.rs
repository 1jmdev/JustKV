...
pub(crate) fn hscan(store: &Store, args: &Args) -> RespFrame {
    let _trace = profiler::scope("crates::commands::src::hash::scan::hscan");
    if args.len() < 3 {
...
fn parse_u64(raw: &[u8]) -> Result<u64, RespFrame> {
    let _trace = profiler::scope("crates::commands::src::hash::scan::parse_u64");
...
fn parse_usize(raw: &[u8]) -> Result<usize, RespFrame> {
    let _trace = profiler::scope("crates::commands::src::hash::scan::parse_usize");
