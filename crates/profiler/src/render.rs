...
pub(crate) fn render_trace(trace: &ActiveTrace) {
    ...
    for (idx, child) in root.children.iter().enumerate() {
        ...
        measure_max_name(trace, *child, "", is_last, &mut max_name);
    }
    ...
    for (idx, child) in root.children.iter().enumerate() {
        let is_last = idx + 1 == root.children.len();
        render_node(trace, *child, "", is_last, root.total_ns, &layout);
    }
...
fn measure_max_name(
    ...
    for (idx, child) in node.children.iter().enumerate() {
        ...
        measure_max_name(trace, *child, &child_prefix, child_last, max);
    }
}
