...
fn get_zset_mut(entry: &mut Entry) -> Option<&mut ZSetValueMap> {
    let _trace = profiler::scope("engine::zset::get_zset_mut");
    entry.as_zset_mut()
}

fn sorted_by_score(map: &ZSetValueMap, reverse: bool) -> Vec<(CompactKey, f64)> {
    let _trace = profiler::scope("engine::zset::sorted_by_score");
...
fn sorted_by_score_refs(map: &ZSetValueMap, reverse: bool) -> Vec<(&CompactKey, f64)> {
    let _trace = profiler::scope("engine::zset::sorted_by_score_refs");
...
fn compare_member_score(
    ...
    let _trace = profiler::scope("engine::zset::compare_member_score");
...
fn normalize_range(start: i64, stop: i64, len: usize) -> Option<(usize, usize)> {
    let _trace = profiler::scope("engine::zset::normalize_range");
    if len == 0 {
        return None;
    }
